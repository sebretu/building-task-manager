...
export async function GET(req: Request) {
  ...
  if (!token) {
    return NextResponse.json({ ok: false, error: { code: 'AUTH_INVALID', message: 'Missing Bearer token' } }, { status: 401 });
  }
  ...
  try {
    const data = await rpcListTasks(supabase, { limit: 10, offset: 0 });
